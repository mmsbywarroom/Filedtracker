...
/// Matches server `punchInWindow.ts`: punch-in from 5:00 AM IST through end of day
/// (including after 1:00 PM). Before 5:00 AM blocked unless unrestricted phone.
enum PunchInWindow {
    private static let startMinutes = 5 * 60
    ...
    static var blockedMessage: String {
        "Punch in is allowed from 5:00 AM IST onward (including after 1:00 PM). Before 5:00 AM is not allowed."
    }
}
