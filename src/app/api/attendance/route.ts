...
/** Face describe on punch can be slow on cold start (native may send image only). */
...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json({ error: "Could not load attendance. Try again." }, { status: 500 });
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    console.error("[punch-in]", e);
    return NextResponse.json(
      { error: "Punch failed. Please try again." },
      { status: 500 }
    );
  }
}
